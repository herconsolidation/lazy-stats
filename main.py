...
if not st.session_state.logged_in:
    # --- LOGIN FORM ---
    st.logo("C:\\Users\\nmbakaza\\Videos\\intelligence\\logo\\logo bg transparent.png")
    ...
    # --- Sidebar ---
    with st.sidebar:
        # Company name + Logo add here
        st.sidebar.image("C:\\Users\\nmbakaza\\Videos\\intelligence\\logo\\logo bg transparent.png", use_container_width=True)

        st.header("Account Details")
        if info:
            #st.success(f"Logged in: {info.name}")
            st.success(f"Logged in: @her.consolidation")
